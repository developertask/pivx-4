// Copyright (c) 2018-2020 The OHONETWORK developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zoho/deterministicmint.h>
#include "zohotracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzOHOTracker::CzOHOTracker(std::string strWalletFile)
{
...
CzOHOTracker::~CzOHOTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzOHOTracker::Init()
{
...
bool CzOHOTracker::Archive(CMintMeta& meta)
{
...
bool CzOHOTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzOHOTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzOHOTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzOHOTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzOHOTracker::GetSerialHashes()
{
...
CAmount CzOHOTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzOHOTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzOHOTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzOHOTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzOHOTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzOHOTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzOHOTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzOHOTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzOHOTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzOHOTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzOHOTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzOHOWallet* zOHOWallet)
{
    bool iszOHOWalletInitialized = (NULL != zOHOWallet);
    CMintMeta meta;
    ...
    if (! iszOHOWalletInitialized)
        zOHOWallet = new CzOHOWallet(strWalletFile);
    meta.isSeedCorrect = zOHOWallet->CheckSeed(dMint);
    if (! iszOHOWalletInitialized)
        delete zOHOWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzOHOTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzOHOTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzOHOTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzOHOTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzOHOTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzOHOTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzOHOWallet* zOHOWallet = new CzOHOWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zOHOWallet);
        }
        delete zOHOWallet;
        LogPrint("zero", "%s: added %d dzoho from DB\n", __func__, listDeterministicDB.size());
    }
