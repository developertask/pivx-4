// Copyright (c) 2017-2019 The OHONETWORK developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zohowallet.h"
#include "main.h"
#include "txdb.h"
...
CzOHOWallet::CzOHOWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zoho seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZOHOSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZOHOSeed_deprecated()) {
                    LogPrintf("%s: Updated zOHO seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zoho seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zoho wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzOHOWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZOHOCount(nCountLastUsed);
    else if (!walletdb.ReadZOHOCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzOHOWallet::Lock()
{
    seedMaster = 0;
}

void CzOHOWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzOHOWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZOHO(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzOHOWallet::LoadMintPoolFromDB()
{
...
void CzOHOWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzOHOWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzOHOWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zohoTracker->HasPubcoinHash(pMint.first)) {
...
bool CzOHOWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZOHO(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zohoTracker which also adds to database
    pwalletMain->zohoTracker->Add(dMint, true);
...
void CzOHOWallet::SeedToZOHO(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzOHOWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzOHOWallet::GenerateDeterministicZOHO(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzOHOWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzOHOWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzOHOWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
