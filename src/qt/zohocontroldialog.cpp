...
#include "ui_zohocontroldialog.h"
...
std::set<std::string> ZOhoControlDialog::setSelectedMints;
std::set<CMintMeta> ZOhoControlDialog::setMints;

bool CZOhoControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZOhoControlDialog::COLUMN_DENOMINATION || column == ZOhoControlDialog::COLUMN_VERSION || column == ZOhoControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZOhoControlDialog::ZOhoControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZOhoControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zOHO Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZOho->setProperty("cssClass", "text-main-purple");
    ui->labelZOho_int->setProperty("cssClass", "text-main-purple");
...
ZOhoControlDialog::~ZOhoControlDialog()
{
    delete ui;
}

void ZOhoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZOhoControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZOhoControlWidgetItem* itemDenom(new CZOhoControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZOhoControlWidgetItem *itemMint = new CZOhoControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zOHO.";
            else if (!mint.isSeedCorrect)
                strReason = "The zOHO seed used to mint this zOHO is not the same as currently hold in the wallet";
            else
...
void ZOhoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZOhoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZOho_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZOhoControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZOhoControlDialog::GetSelectedMints()
{
...
void ZOhoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
