...
bool WalletModel::isLiteNodeStakingNetworkelyEnabled() const {
...
CAmount WalletModel::getMinLiteNodeStakingAmount() const
{
    return Params().GetMinLiteNodeStakingAmount();
}
...
{
    return wallet->GetLiteNodeStakingBalance();
}

bool WalletModel::isLiteNodeStaking() const
{
    // TODO: Complete me..
    return false;
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered ohonetwork address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > sporkManager.GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 OHO.").arg(sporkManager.GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
    ...
    if(!CheckTransaction(*transaction.getTransaction(), true, true, state, true, fLiteNodeStakingActive)){
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal ohonetwork:URI (ohonetwork:XyZ...?message=example)
            {
...
bool WalletModel::createZohoSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZoho(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
...
static void NotifyzOHOReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzOHOReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzOHOReset.connect(boost::bind(NotifyzOHOReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzOHOReset.disconnect(boost::bind(NotifyzOHOReset, this));
...
bool WalletModel::whitelistAddressFromLiteNodeStaking(const QString &addressStr) {
    return updateAddressBookPurpose(addressStr, AddressBook::AddressBookPurpose::DELEGATOR);
}

bool WalletModel::blacklistAddressFromLiteNodeStaking(const QString &addressStr) {
...
bool WalletModel::updateAddressBookPurpose(const QString &addressStr, const std::string& purpose) {
    ...
    if (address.IsStakingAddress())
        return error("Invalid OHONETWORK address, cold staking address");
    CKeyID keyID;
...
bool WalletModel::getKeyId(const CBitcoinAddress& address, CKeyID& keyID) {
    if (!address.IsValid())
        return error("Invalid OHONETWORK address");

    if (!address.GetKeyID(keyID))
        return error("Unable to get KeyID from OHONETWORK address");
...
{
    setMints.clear();
    setMints = pwalletMain->zohoTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
