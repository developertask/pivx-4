                                                        ...
                                                        model(0)
{
    ...
    ui->labelTitle->setText("Select OHO Denominations to Spend");
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::OHO;
    ...
    QString toolTip4 = tr("Can vary +/- %1 uoho per input.").arg(dFeeVary);
    ...
    for (PAIRTYPE(QString, std::vector<COutput>) coins : mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show OHONETWORK address. In tree mode, address is not shown again for direct wallet address outputs
