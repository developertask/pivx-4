...
#include "qt/ohonetwork/forms/ui_addresseswidget.h"
#include "qt/ohonetwork/addresslabelrow.h"
#include "qt/ohonetwork/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(OHONETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter OHONETWORK address"));
...
void AddressesWidget::loadWalletModel(){
    if(walletModel) {
        ...
        this->filter = new AddressFilterProxyModel(QStringList({AddressTableModel::Send, AddressTableModel::LiteNodeStakingSend}), this);
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress ohoAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(ohoAdd.Get(), label.toUtf8().constData(),
                ohoAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
