...
#include "qt/ohonetwork/privacywidget.h"
#include "qt/ohonetwork/forms/ui_privacywidget.h"
#include "qt/ohonetwork/qtutils.h"
#include "guiutil.h"
#include "qt/ohonetwork/denomgenerationdialog.h"
...
PrivacyWidget::PrivacyWidget(OHONETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zOHO anonymizes your OHO by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zOHO or convert back to OHO"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 OHO ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zOHO");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zOHO");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zOHO");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zOHO");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zOHO");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zOHO");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zOHO");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zOHO");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zOHO Movements"));
    ...
    // Only Convert to OHO enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzOHO->setTitleClassAndText("btn-title-grey", "Total 0 zOHO");
    ui->btnTotalzOHO->setSubTitleClassAndText("text-subtitle", "Show denominations of zOHO owned.");
    ui->btnTotalzOHO->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select OHO outputs to mint into zOHO.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zOHO");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzOHO, SIGNAL(clicked()), this, SLOT(onTotalZohoClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zOHO");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of OHO to mint into zOHO"));
    }else{
        btnText = tr("Convert back to OHO");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zOHO to convert back into OHO"));
    }
...
void PrivacyWidget::onTotalZohoClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzOHO->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzOHO->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zOHO is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zOHO, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zOHO").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zOHO minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZoho(
            value,
            selectedMints,
            mintChange,
            minimizeChange,
        ...
        // Spend succeed
        inform(tr("zOHO converted back to OHO"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zOHO </b>";
    ...
    ui->btnTotalzOHO->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
