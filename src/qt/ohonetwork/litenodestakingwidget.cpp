// Copyright (c) 2019-2020 The OHONETWORK developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "qt/ohonetwork/litenodestakingwidget.h"
#include "qt/ohonetwork/forms/ui_litenodestakingwidget.h"
#include "qt/ohonetwork/qtutils.h"
#include "amount.h"
#include "guiutil.h"
#include "qt/ohonetwork/requestdialog.h"
#include "qt/ohonetwork/tooltipmenu.h"
#include "qt/ohonetwork/furlistrow.h"
#include "qt/ohonetwork/sendconfirmdialog.h"
#include "qt/ohonetwork/addnewcontactdialog.h"
#include "qt/ohonetwork/guitransactionsutils.h"
#include "walletmodel.h"
#include "optionsmodel.h"
#include "coincontroldialog.h"
#include "coincontrol.h"
#include "qt/ohonetwork/csrow.h"
...
public:
    ...
    void init(QWidget* holder,const QModelIndex &index, bool isHovered, bool isSelected) const override{
        ...
        QString label = index.sibling(index.row(), LiteNodeStakingModel::OWNER_ADDRESS_LABEL).data(Qt::DisplayRole).toString();
        if (label.isEmpty()) {
            label = "Address with no label";
        }
        bool isWhitelisted = index.sibling(index.row(), LiteNodeStakingModel::IS_WHITELISTED).data(Qt::DisplayRole).toBool();
        QString amountStr = index.sibling(index.row(), LiteNodeStakingModel::TOTAL_STACKEABLE_AMOUNT_STR).data(Qt::DisplayRole).toString();
        bool isReceivedDelegation = index.sibling(index.row(), LiteNodeStakingModel::IS_RECEIVED_DELEGATION).data(Qt::DisplayRole).toBool();
...
LiteNodeStakingWidget::LiteNodeStakingWidget(OHONETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::LiteNodeStakingWidget),
    isLoading(false)
{
    ui->setupUi(this);
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can delegate your OHOs, letting a hot node (24/7 online node)\nstake on your behalf, while you keep the keys securely offline."));
    ...
    ui->labelSubtitle2->setText(tr("Accept OHO delegation / Delegate OHO"));
    ...
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select OHO outputs to delegate.");

    ui->btnLiteNodeStaking->setTitleClassAndText("btn-title-grey", "Create Cold Staking Address");
    ui->btnLiteNodeStaking->setSubTitleClassAndText("text-subtitle", "Creates an address to receive delegated coins\nand stake them on their owner's behalf.");
    ui->btnLiteNodeStaking->layout()->setMargin(0);

    connect(ui->btnCoinControl, SIGNAL(clicked()), this, SLOT(onCoinControlClicked()));
    connect(ui->btnLiteNodeStaking, SIGNAL(clicked()), this, SLOT(onColdStakeClicked()));
    ...
    connect(ui->pushButtonSend, &QPushButton::clicked, this, &LiteNodeStakingWidget::onSendClicked);
...
void LiteNodeStakingWidget::loadWalletModel(){
    if(walletModel) {
        sendMultiRow->setWalletModel(walletModel);
        txModel = walletModel->getTransactionTableModel();
        csModel = new LiteNodeStakingModel(walletModel, txModel, walletModel->getAddressTableModel(), this);
        ui->listView->setModel(csModel);
        ui->listView->setModelColumn(LiteNodeStakingModel::OWNER_ADDRESS);

        addressTableModel = walletModel->getAddressTableModel();
        addressesFilter = new AddressFilterProxyModel(AddressTableModel::LiteNodeStaking, this);
        ...
        connect(txModel, &TransactionTableModel::txArrived, this, &LiteNodeStakingWidget::onTxArrived);
...
void LiteNodeStakingWidget::onTxArrived(const QString& hash, const bool& isCoinStake, const bool& isCSAnyType) {
    if (isCSAnyType) {
        tryRefreshDelegations();
    }
}

void LiteNodeStakingWidget::walletSynced(bool sync) {
...
void LiteNodeStakingWidget::tryRefreshDelegations() {
...
bool LiteNodeStakingWidget::refreshDelegations(){
    if (isLoading) return false;
    isLoading = true;
    return execute(REQUEST_LOAD_TASK);
}

void LiteNodeStakingWidget::onDelegationsRefreshed() {
    isLoading = false;
...
void LiteNodeStakingWidget::run(int type) {
    if (type == REQUEST_LOAD_TASK) {
        csModel->updateCSList();
        QMetaObject::invokeMethod(this, "onDelegationsRefreshed", Qt::QueuedConnection);
    }
}
void LiteNodeStakingWidget::onError(QString error, int type) {
    isLoading = false;
    inform(tr("Error loading delegations: %1").arg(error));
}

void LiteNodeStakingWidget::onContactsClicked(bool ownerAdd) {
    isContactOwnerSelected = ownerAdd;
    onContactsClicked();
}

void LiteNodeStakingWidget::onContactsClicked(){
    ...
    menuContacts->setWalletModel(walletModel, isContactOwnerSelected ? AddressTableModel::Receive : AddressTableModel::LiteNodeStakingSend);
...
void LiteNodeStakingWidget::onDelegateSelected(bool delegate){
    ...
    if(isInDelegation){
        ...
        ui->listView->setVisible(false);
        ui->containerHistoryLabel->setVisible(false);
        ui->btnLiteNodeStaking->setVisible(false);
        ...
        ui->btnLiteNodeStaking->setVisible(true);
...
void LiteNodeStakingWidget::onSendClicked(){
    ...
    // Amount must be >= minLiteNodeStakingAmount
    const CAmount& minLiteNodeStakingAmount = walletModel->getMinLiteNodeStakingAmount();
    if (dest.amount < minLiteNodeStakingAmount) {
        inform(tr("Invalid entry, minimum delegable amount is ") +
               BitcoinUnits::formatWithUnit(nDisplayUnit, minLiteNodeStakingAmount));
        return;
    }
...
void LiteNodeStakingWidget::clearAll() {
...
void LiteNodeStakingWidget::onCoinControlClicked(){
    if(isInDelegation) {
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any OHO to select."));
        }
    }
}

void LiteNodeStakingWidget::onColdStakeClicked() {
    showAddressGenerationDialog(false);
}

void LiteNodeStakingWidget::showAddressGenerationDialog(bool isPaymentRequest) {
...
void LiteNodeStakingWidget::handleMyColdAddressClicked(const QModelIndex &_index) {
...
void LiteNodeStakingWidget::handleAddressClicked(const QModelIndex &rIndex) {

    bool isReceivedDelegation = rIndex.sibling(rIndex.row(), LiteNodeStakingModel::IS_RECEIVED_DELEGATION).data(Qt::DisplayRole).toBool();
    ...
    if (isReceivedDelegation) {
        bool isWhitelisted = rIndex.sibling(rIndex.row(), LiteNodeStakingModel::IS_WHITELISTED).data(
...
void LiteNodeStakingWidget::onAddressCopyClicked() {
    GUIUtil::setClipboard(addressIndex.data(Qt::DisplayRole).toString());
    inform(tr("Address copied"));
}
void LiteNodeStakingWidget::onAddressEditClicked() {
    onLabelClicked(
...
void LiteNodeStakingWidget::onEditClicked() {
    // whitelist address
...
void LiteNodeStakingWidget::onDeleteClicked() {
    // blacklist address
...
void LiteNodeStakingWidget::onCopyClicked() {
    // show address info
}

void LiteNodeStakingWidget::onCopyOwnerClicked() {
    QString owner = index.data(Qt::DisplayRole).toString();
    GUIUtil::setClipboard(owner);
    inform(tr("Owner address copied"));
}

void LiteNodeStakingWidget::onLabelClicked(){
    onLabelClicked(
...
void LiteNodeStakingWidget::onLabelClicked(QString dialogTitle, const QModelIndex &index, const bool& isMyLiteNodeStakingAddresses) {
    if(walletModel && !isShowingDialog) {
        isShowingDialog = true;
        showHideOp(true);
        ...
        if (openDialogWithOpaqueBackgroundY(dialog, window, 3.5, 6)) {
            ...
            if (!label.isEmpty() && walletModel->updateAddressBookLabels(
                ...
                if (isMyLiteNodeStakingAddresses) {
...
void LiteNodeStakingWidget::onMyStakingAddressesClicked(){
...
void LiteNodeStakingWidget::changeTheme(bool isLightTheme, QString& theme){
...
void LiteNodeStakingWidget::updateStakingTotalLabel()
{
    const CAmount& total = csModel->getTotalAmount();
    ui->labelStakingTotal->setText(tr("Total Staking: %1").arg(
            (total == 0) ? "0.00 OHO" : GUIUtil::formatBalance(total, nDisplayUnit))
    );
}

LiteNodeStakingWidget::~LiteNodeStakingWidget(){
    if (sendMultiRow)
        delete sendMultiRow;
    if (txHolder)
        delete txHolder;
