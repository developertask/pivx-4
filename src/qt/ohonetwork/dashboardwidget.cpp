...
#include "qt/ohonetwork/dashboardwidget.h"
#include "qt/ohonetwork/forms/ui_dashboardwidget.h"
#include "qt/ohonetwork/sendconfirmdialog.h"
#include "qt/ohonetwork/txrow.h"
...
DashboardWidget::DashboardWidget(OHONETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of OHO and zOHO staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareOho, "square-chart-oho");
    setCssProperty(ui->labelSquarezOho, "square-chart-zoho");
    setCssProperty(ui->labelOho, "text-chart-oho");
    setCssProperty(ui->labelZoho, "text-chart-zoho");
    ...
    ui->labelAmountZoho->setText("0 zOHO");
    ui->labelAmountOho->setText("0 OHO");
    setCssProperty(ui->labelAmountOho, "text-stake-oho-disable");
    setCssProperty(ui->labelAmountZoho, "text-stake-zoho-disable");
    ...
    ui->btnHowTo->setText(tr("How to get OHO or zOHO"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("OHO")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isOho = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZOHO;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair OHO, zOHO
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            oho = (pair.first != 0) ? pair.first / 100000000 : 0;
            zoho = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalOho += pair.first;
            chartData->totalZoho += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesOho.append(oho);
        chartData->valueszOho.append(zoho);

        int max = std::max(oho, zoho);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("OHO");
    set1 = new QBarSet("zOHO");
    ...
    set0->append(chartData->valuesOho);
    set1->append(chartData->valueszOho);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalOho > 0 || chartData->totalZoho > 0) {
        setCssProperty(ui->labelAmountOho, "text-stake-oho");
        setCssProperty(ui->labelAmountZoho, "text-stake-zoho");
    } else {
        setCssProperty(ui->labelAmountOho, "text-stake-oho-disable");
        setCssProperty(ui->labelAmountZoho, "text-stake-zoho-disable");
    }
    forceUpdateStyle({ui->labelAmountOho, ui->labelAmountZoho});
    ui->labelAmountOho->setText(GUIUtil::formatBalance(chartData->totalOho, nDisplayUnit));
    ui->labelAmountZoho->setText(GUIUtil::formatBalance(chartData->totalZoho, nDisplayUnit, true));
