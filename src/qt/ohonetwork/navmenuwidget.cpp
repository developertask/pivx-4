...
#include "qt/ohonetwork/navmenuwidget.h"
#include "qt/ohonetwork/forms/ui_navmenuwidget.h"
...
NavMenuWidget::NavMenuWidget(OHONETWORKGUI *mainWindow, QWidget *parent) :
    ...
    ui(new Ui::NavMenuWidget)
{
    ...
    ui->btnLiteNodeStaking->setProperty("name", "cold-staking");
    ui->btnLiteNodeStaking->setText("COLD\r\nSTAKING");
    ui->btnLiteNodeStaking->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
    ...
    btns = {ui->btnDashboard, ui->btnSend, ui->btnReceive, ui->btnAddress, ui->btnPrivacy, ui->btnMaster, ui->btnLiteNodeStaking, ui->btnSettings, ui->btnLiteNodeStaking};
...
void NavMenuWidget::loadWalletModel() {
    if (walletModel && walletModel->getOptionsModel()) {
        ui->btnLiteNodeStaking->setVisible(walletModel->getOptionsModel()->isLiteNodeStakingScreenEnabled());
    }
}
...
void NavMenuWidget::connectActions() {
    ...
    connect(ui->btnLiteNodeStaking,SIGNAL(clicked()),this, SLOT(onLiteNodeStakingClicked()));
    ...
    ui->btnLiteNodeStaking->setShortcut(QKeySequence(SHORT_KEY + Qt::Key_7));
...
void NavMenuWidget::onLiteNodeStakingClicked() {
    window->goToLiteNodeStaking();
    onNavSelected(ui->btnLiteNodeStaking);
}
...
void NavMenuWidget::onShowHideLiteNodeStakingChanged(bool show) {
    ui->btnLiteNodeStaking->setVisible(show);
    if (show)
        ui->scrollAreaNav->verticalScrollBar()->setValue(ui->btnLiteNodeStaking->y());
}
