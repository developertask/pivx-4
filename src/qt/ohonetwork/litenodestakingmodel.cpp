...
LiteNodeStakingModel::LiteNodeStakingModel(WalletModel* _model,
...
bool LiteNodeStakingModel::parseCSDelegation(const CTxOut& out, CSDelegation& ret, const QString& txId, const int& utxoIndex) {
    txnouttype type;
...
int LiteNodeStakingModel::rowCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return cachedDelegations.size();
}

int LiteNodeStakingModel::columnCount(const QModelIndex &parent) const
{
    Q_UNUSED(parent);
    return COLUMN_COUNT;
}
...
QVariant LiteNodeStakingModel::data(const QModelIndex &index, int role) const
{
    if(!index.isValid())
            return QVariant();
...
bool LiteNodeStakingModel::whitelist(const QModelIndex& modelIndex)
{
    ...
    if (!model->whitelistAddressFromLiteNodeStaking(address)) return false;
...
bool LiteNodeStakingModel::blacklist(const QModelIndex& modelIndex)
{
    QString address = modelIndex.data(Qt::DisplayRole).toString();
    if (!addressTableModel->isWhitelisted(address.toStdString())) {
    ...
    if (!model->blacklistAddressFromLiteNodeStaking(address)) return false;
...
void LiteNodeStakingModel::removeRowAndEmitDataChanged(const int idx)
{
