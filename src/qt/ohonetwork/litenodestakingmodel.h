...
class LiteNodeStakingModel : public QAbstractTableModel
{
    Q_OBJECT

public:
    explicit LiteNodeStakingModel(WalletModel* model, TransactionTableModel* _tableModel, AddressTableModel* _addressTableModel, QObject *parent = nullptr);
    ~LiteNodeStakingModel() override {
        model = nullptr;
        tableModel = nullptr;
