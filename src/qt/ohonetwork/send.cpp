...
#include "qt/ohonetwork/send.h"
#include "qt/ohonetwork/forms/ui_send.h"
#include "qt/ohonetwork/addnewcontactdialog.h"
#include "qt/ohonetwork/qtutils.h"
#include "qt/ohonetwork/sendchangeaddressdialog.h"
#include "qt/ohonetwork/optionbutton.h"
#include "qt/ohonetwork/sendconfirmdialog.h"
#include "qt/ohonetwork/myaddressrow.h"
...
#include "zohocontroldialog.h"

SendWidget::SendWidget(OHONETWORKGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("OHO");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zOHO");
    setCssProperty(ui->pushRight, "btn-check-right");

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (OHO) or private coins (zOHO)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("OHONETWORK address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 OHO");
    ...
    setCssProperty(coinIcon, "coin-icon-oho");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onOHOSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onOHOSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZoho = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZoho));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZoho ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendOho = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendOho ? AskPassphraseDialog::Context::Send_OHO : AskPassphraseDialog::Context::Send_zOHO, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendOho) ? send(recipients) : sendZoho(recipients)) {
...
bool SendWidget::sendZoho(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        emit message(tr("Spend Zerocoin"), tr("zOHO is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zOHO selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZOhoControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZOhoControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZoho(
        ...
        inform(tr("zOHO transaction sent!"));
        ZOhoControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZOHO_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zOHO require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zOHO transaction
        ...
        emit message("zOHO transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isOHO){
        ...
        if (walletModel->getZerocoinBalance() > 0) {
            ZOhoControlDialog *zOhoControl = new ZOhoControlDialog(this);
            zOhoControl->setModel(walletModel);
            zOhoControl->exec();
            ui->btnCoinControl->setActive(!ZOhoControlDialog::setSelectedMints.empty());
            zOhoControl->deleteLater();
        } else {
            inform(tr("You don't have any zOHO in your balance to select."));
        }
    }
}
...
void SendWidget::onOHOSelected(bool _isOHO){
    isOHO = _isOHO;
    setCssProperty(coinIcon, _isOHO ? "coin-icon-oho" : "coin-icon-zoho");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress ohoAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(ohoAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ohoAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
