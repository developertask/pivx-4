...
#include "qt/ohonetwork/topbar.h"
#include "qt/ohonetwork/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/ohonetwork/lockunlock.h"
#include "qt/ohonetwork/qtutils.h"
#include "qt/ohonetwork/receivedialog.h"
...
TopBar::TopBar(OHONETWORKGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopOho, ui->labelAmountTopzOho}, "amount-small-topbar");
    setCssProperty({ui->labelAmountOho, ui->labelAmountzOho}, "amount-topbar");
    setCssProperty({ui->labelPendingOho, ui->labelPendingzOho, ui->labelImmatureOho, ui->labelImmaturezOho}, "amount-small-topbar");
    ...
    ui->pushButtonLiteNodeStaking->setButtonClassStyle("cssClass", "btn-check-cold-staking-inactive");
    ui->pushButtonLiteNodeStaking->setButtonText("Cold Staking Disabled");
    ...
    connect(ui->pushButtonLiteNodeStaking, SIGNAL(Mouse_Pressed()), this, SLOT(onLiteNodeStakingClicked()));
...
void TopBar::onLiteNodeStakingClicked() {

    bool isLiteNodeStakingEnabled = walletModel->isLiteNodeStaking();
    ui->pushButtonLiteNodeStaking->setChecked(isLiteNodeStakingEnabled);

    bool show = (isInitializing) ? walletModel->getOptionsModel()->isLiteNodeStakingScreenEnabled() :
            walletModel->getOptionsModel()->invertLiteNodeStakingScreenStatus();
    QString className;
    QString text;

    if (isLiteNodeStakingEnabled) {
    ...
    ui->pushButtonLiteNodeStaking->setButtonClassStyle("cssClass", className, true);
    ui->pushButtonLiteNodeStaking->setButtonText(text);
    updateStyle(ui->pushButtonLiteNodeStaking);

    emit onShowHideLiteNodeStakingChanged(show);
}
...
void TopBar::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("OHONETWORK")
    updateDisplayUnit();

    refreshStatus();
    onLiteNodeStakingClicked();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // OHO Total
    CAmount ohoAvailableBalance = balance;
    // zOHO Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalOho = GUIUtil::formatBalance(ohoAvailableBalance, nDisplayUnit);
    QString totalzOho = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopOho->setText(totalOho);
    ui->labelAmountTopzOho->setText(totalzOho);

    // Expanded
    ui->labelAmountOho->setText(totalOho);
    ui->labelAmountzOho->setText(totalzOho);

    ui->labelPendingOho->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelPendingzOho->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmatureOho->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezOho->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
