...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.HasZerocoinSpendInputs()) {
        libzerocoin::CoinSpend zcspend = wtx.HasZerocoinPublicSpendInputs() ? ZOHOModule::parseCoinSpend(wtx.vin[0]) : TxInToZerocoinSpend(wtx.vin[0]);
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.HasZerocoinSpendInputs() && (fZSpendFromMe || wallet->zohoTracker->HasMintTx(hash))) {
            //zOHO stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZOHO;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime, wtx.GetTotalSize());
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zOho;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by OHONETWORK Address
...
void TransactionRecord::loadUnlockColdStake(const CWallet* wallet, const CWalletTx& wtx, TransactionRecord& record)
{
    ...
    for (const auto &input : wtx.vin) {
        ...
        if (tx && tx->vout[input.prevout.n].scriptPubKey.IsPayToLiteNodeStaking()) {
    ...
    if (isSpendable) {
        ...
        record.debit = -(wtx.GetLiteNodeStakingDebit());
        record.credit = -(wtx.GetLiteNodeStakingCredit());
    }
...
bool IsZOHOType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZOHO:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zOho:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    // For generated transactions, determine maturity
            ...
            type == TransactionRecord::StakeZOHO ||
...
{
    return (type == TransactionRecord::StakeMint || type == TransactionRecord::Generated || type == TransactionRecord::StakeZOHO);
}

bool TransactionRecord::isAnyLiteNodeStakingType() const
{
