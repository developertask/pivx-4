...
const QString AddressTableModel::LiteNodeStaking = "C";
const QString AddressTableModel::LiteNodeStakingSend = "Csend";
...
static AddressTableEntry::Type translateTransactionType(const QString& strPurpose, bool isMine)
{
    ...
    if (strPurpose ==  QString::fromStdString(AddressBook::AddressBookPurpose::SEND))
        ...
        addressType = AddressTableEntry::LiteNodeStaking;
    else if (strPurpose == QString::fromStdString(AddressBook::AddressBookPurpose::COLD_STAKING_SEND))
        addressType = AddressTableEntry::LiteNodeStakingSend;
        ...
        cachedAddressTable.clear();
        {
            ...
            for (const PAIRTYPE(CTxDestination, AddressBook::CAddressBookData) & item : wallet->mapAddressBook) {
                ...
                const CChainParams::Base58Type addrType =
                        AddressBook::IsLiteNodeStakingPurpose(item.second.purpose) ?
    ...
    if (role == Qt::DisplayRole || role == Qt::EditRole) {
        ...
        switch (rec->type) {
            ...
            case AddressTableEntry::Delegators:
                return Delegators;
            case AddressTableEntry::LiteNodeStaking:
                return LiteNodeStaking;
            case AddressTableEntry::LiteNodeStakingSend:
                return LiteNodeStakingSend;
            default:
                break;
        }
    }
...
void AddressTableModel::updateEntry(const QString& address,
    ...
    int status)
{
    // Update address book model from Ohonetwork core
...
bool AddressTableModel::removeRows(int row, int count, const QModelIndex& parent)
{
    ...
    if (count != 1 || !rec || rec->type == AddressTableEntry::Receiving || rec->type == AddressTableEntry::LiteNodeStaking) {
        // Can only remove one row at a time, and cannot remove rows not in model.
        // Also refuse to remove receiving addresses.
        return false;
    }
    const CChainParams::Base58Type addrType = (rec->type == AddressTableEntry::LiteNodeStakingSend) ? CChainParams::STAKING_ADDRESS : CChainParams::PUBKEY_ADDRESS;
    {
        LOCK(wallet->cs_wallet);
