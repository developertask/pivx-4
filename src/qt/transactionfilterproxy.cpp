...
bool TransactionFilterProxy::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
{
    ...
    if (fOnlyLiteNodeStaking && !isColdStake(type))
        return false;
...
{
    this->fOnlyLiteNodeStaking = fOnlyColdStakes;
    invalidateFilter();
}
...
bool TransactionFilterProxy::isOrphan(const int status, const int type)
{
    return ( (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint ||
            type == TransactionRecord::StakeZOHO || type == TransactionRecord::MNReward)
...
bool TransactionFilterProxy::isZcTx(int type) const {
    return (type == TransactionRecord::ZerocoinMint || type == TransactionRecord::ZerocoinSpend || type == TransactionRecord::ZerocoinSpend_Change_zOho
