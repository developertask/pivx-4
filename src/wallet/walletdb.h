...
public:
    ...
    bool WriteZOHOSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZOHOSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZOHOSeed_deprecated(uint256& seed);
    bool EraseZOHOSeed();
    bool EraseZOHOSeed_deprecated();

    bool WriteZOHOCount(const uint32_t& nCount);
    bool ReadZOHOCount(uint32_t& nCount);
