...
#include "zoho/zohowallet.h"
#include "zoho/zohotracker.h"
...
/**
 * Fees smaller than this (in uoho) are considered zero fee (for transaction creation)
...
{
    return zohoTracker->HasSerial(bnSerial);
}
...
CAmount CWalletTx::GetLiteNodeStakingDebit(bool fUseCache) const
{
...
CAmount CWalletTx::GetLiteNodeStakingCredit(bool fUseCache) const
{
...
int CWallet::ScanForWalletTransactions(CBlockIndex* pindexStart, bool fUpdate, bool fromStartup)
{
    ...
    bool fCheckZOHO = GetBoolArg("-zapwallettxes", false);
    if (fCheckZOHO)
        zohoTracker->Init();
    ...
    CBlockIndex* pindex = pindexStart;
    {
        ...
        while (pindex) {
            ...
            //If this is a zapwallettx, need to readd zoho
            if (fCheckZOHO && pindex->nHeight >= Params().Zerocoin_StartHeight()) {
...
CAmount CWallet::GetLiteNodeStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        if (pcoin.HasP2CSOutputs() && pcoin.IsTrusted())
            nTotal += pcoin.GetLiteNodeStakingCredit();
    });
}

CAmount CWallet::GetStakingBalance(const bool fIncludeLiteNodeStaking) const
{
    return GetBalance() + (fIncludeLiteNodeStaking ? GetLiteNodeStakingBalance() : 0);
}
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zOHO in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zohoTracker->GetMints(true);
    ...
    return zohoTracker->GetBalance(false, false);
}
...
{
    return zohoTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zohoTracker->ListMints(true, true, true);
...
CAmount CWallet::GetImmatureLiteNodeStakingBalance() const
{
    return loopTxsBalance([](const uint256& id, const CWalletTx& pcoin, CAmount& nTotal) {
        ...
        for (const auto& it : mapWallet) {
            ...
            if (pcoin->HasP2CSOutputs()) {
                for (int i = 0; i < (int) pcoin->vout.size(); i++) {
                    ...
                    if (utxo.scriptPubKey.IsPayToLiteNodeStaking()) {
...

    {
        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                // --Skip P2CS outputs
                // skip cold coins
                if (mine == ISMINE_COLD && (!fIncludeLiteNodeStaking || !HasDelegator(pcoin->vout[i]))) continue;
                // skip delegated coins
                if (mine == ISMINE_SPENDABLE_DELEGATED && !fIncludeDelegated) continue;
                // skip auto-delegated coins
                if (mine == ISMINE_SPENDABLE_STAKEABLE && !fIncludeLiteNodeStaking && !fIncludeDelegated) continue;

                bool fIsValid = (
                        ((mine & ISMINE_SPENDABLE) != ISMINE_NO) ||
                        ((mine & (ISMINE_MULTISIG | (fIncludeLiteNodeStaking ? ISMINE_COLD : ISMINE_NO) |
...
std::map<CBitcoinAddress, std::vector<COutput> > CWallet::AvailableCoinsByAddress(bool fConfirmed, CAmount maxCoinValue)
{
    ...
    for (COutput out : vCoins) {
        ...
        if (!ExtractDestination(out.tx->vout[out.i].scriptPubKey, address, fColdStakeAddr)) {
            ...
            fColdStakeAddr = true;
            if ( !out.tx->vout[out.i].scriptPubKey.IsPayToLiteNodeStaking() ||
...
bool CWallet::SelectStakeCoins(std::list<std::unique_ptr<CStakeInput> >& listInputs, CAmount nTargetAmount, int blockHeight)
{
    ...
    const bool fIncludeCold = sporkManager.IsSporkActive(SPORK_17_COLDSTAKING_ENFORCEMENT) && GetBoolArg("-litenodestaking", true);
    AvailableCoins(vCoins, true, NULL, false, STAKABLE_COINS, false, 1, fIncludeCold, false);

    CAmount nAmountSelected = 0;
    if (GetBoolArg("-ohostake", true)) {
        for (const COutput &out : vCoins) {
            ...
            std::unique_ptr<COhoStake> input(new COhoStake());
...
{
    LOCK(cs_main);
    CAmount nBalance = GetStakingBalance(GetBoolArg("-litenodestaking", true));
    CAmount nZohoBalance = GetZerocoinBalance(false);
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX, bool fIncludeLiteNodeStaking, bool fIncludeDelegated) const
{
    // Note: this function should never be used for "always free" tx types like dstx
    std::vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, false, coin_type, useIX, 1, fIncludeLiteNodeStaking, fIncludeDelegated);
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 OHO collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 OHO collateral
            ...
            while (true) {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX, false, fIncludeDelegated)) {
                    if (coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 10000 OHO.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT10000IFMN) {
                        strFailReason = _("Unable to locate enough Obfuscation non-denominated funds for this transaction that are not equal 10000 OHO.");
                    } else {
                ...
                for (PAIRTYPE(const CWalletTx*, unsigned int) pcoin : setCoins) {
                    if(pcoin.first->vout[pcoin.second].scriptPubKey.IsPayToLiteNodeStaking())
                ...
                if (nChange > 0) {
                    ...
                    // change transaction isn't always pay-to-ohonetwork-address
                    CScript scriptChange;
        ...
        )
{
    ...
    for (std::unique_ptr<CStakeInput>& stakeInput : listInputs) {
        ...
        //Masternode payment
        FillBlockPayee(txNew, nMinFee, true, stakeInput->IsZOHO());
        ...
        //Mark mints as spent
        if (stakeInput->IsZOHO()) {
            CZOhoStake* z = (CZOhoStake*)stakeInput.get();
...
CBitcoinAddress CWallet::ParseIntoAddress(const CTxDestination& dest, const std::string& purpose) {
    const CChainParams::Base58Type addrType =
            AddressBook::IsLiteNodeStakingPurpose(purpose) ?
...
void CWallet::CreateAutoMintTransaction(const CAmount& nMintAmount, CCoinControl* coinControl)
{
    if (nMintAmount > 0){
        ...
        LogPrintf("CWallet::AutoZeromint() @ block %ld: successfully minted %ld zOHO. Current percentage of zOHO: %lf%%\n",
...
void CWallet::AutoZeromint()
{
    ...
    // zOHO are integers > 0, so we can't mint 10% of 9 OHO
    if (nBalance < 10){
        LogPrint("zero", "CWallet::AutoZeromint(): available balance (%ld) too small for minting zOHO\n", nBalance);
        return;
    }

    // Percentage of zOHO we already have
    ...
    if(dPercentage >= nZeromintPercentage){
        LogPrint("zero", "CWallet::AutoZeromint() @block %ld: percentage of existing zOHO (%lf%%) already >= configured percentage (%d%%). No minting needed...\n",
                  chainActive.Tip()->nHeight, dPercentage, nZeromintPercentage);
        return;
    }

    // zOHO amount needed for the target percentage
    nToMintAmount = ((nZerocoinBalance + nBalance) * nZeromintPercentage / 100);

    // zOHO amount missing from target (must be minted)
    nToMintAmount = (nToMintAmount - nZerocoinBalance) / COIN;

    // Use the biggest denomination smaller than the needed zOHO We'll only mint exact denomination to make minting faster.
    ...
    // If a preferred denomination is used (means nPreferredDenom != 0) do nothing until we have enough OHO to mint this denomination
...
bool CWallet::CreateZOHOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwalletMain->GenerateDeterministicZOHO(denomination, coin, dMint);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, std::vector<CDeterministicMint>& vDMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateZOHOOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zoho output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to Oho
    ...
    // Sign if these are ohonetwork outputs - NOTE that zOHO outputs are signed later in SoK
    if (!isZCSpendChange) {
        int nIn = 0;
        for (const std::pair<const CWalletTx*, unsigned int>& coin : setCoins) {
            if (!SignSignature(*this, *coin.first, txNew, nIn++)) {
...
bool CWallet::CheckCoinSpend(libzerocoin::CoinSpend& spend, libzerocoin::Accumulator& accumulator, CZerocoinSpendReceipt& receipt)
{
    if (!spend.Verify(accumulator)) {
        receipt.SetStatus(_("The transaction did not verify"), ZOHO_BAD_SERIALIZATION);
    ...
    if (Params().NetworkID() != CBaseChainParams::REGTEST && IsSerialKnown(spend.getCoinSerialNumber())) {
        //Tried to spend an already spent zOHO
        receipt.SetStatus(_("The coin spend has been used"), ZOHO_SPENT_USED_ZOHO);
        uint256 hashSerial = GetSerialHash(spend.getCoinSerialNumber());
        if(!zohoTracker->HasSerialHash(hashSerial))
            return error("%s: serialhash %s not found in tracker", __func__, hashSerial.GetHex());

        CMintMeta meta = zohoTracker->Get(hashSerial);
        meta.isUsed = true;
        if (!zohoTracker->UpdateState(meta))
                         ...
                         CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZOHO_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GenerateAccumulatorWitness(&coinWitness, mapAccumulators, pindexCheckpoint)) {
            receipt.SetStatus(_("Couldn't generate the accumulator witness"),
                              ZOHO_FAILED_ACCUMULATOR_INITIALIZATION);
        ...
        if (nVersion >= libzerocoin::PrivateCoin::PUBKEY_VERSION) {
            ...
            if (!mint.GetKeyPair(key))
                return error("%s: failed to set zOHO privkey mint version=%d", __func__, nVersion);
        ...
        try {
            ...
            if (!CheckCoinSpend(spend, accumulator, receipt)) {
                receipt.SetStatus(_("CoinSpend: failed check"), ZOHO_SPEND_ERROR);
                return error("%s : %s", __func__, receipt.GetStatusMessage());
            }
            ...
            receipt.SetStatus(_("CoinSpend: Accumulator witness does not verify"), ZOHO_INVALID_WITNESS);
            return error("%s : %s", __func__, receipt.GetStatusMessage());
        }
    }
    ...
    receipt.SetStatus(_("Spend Valid"), ZOHO_SPEND_OKAY); // Everything okay
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZOHO_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZOHO_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZOHO_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZOHOModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZOHO_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
        ...
        bool isPublicSpend)
{
    ...
    int nStatus = ZOHO_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zOHO"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZOHO_TRX_CREATE;
    ...
    const int nMaxSpends = Params().Zerocoin_MaxPublicSpendsPerTransaction(); // Maximum possible spends for one zOHO public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zOHO used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zohoTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zOHO mints to use in this spend
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CBitcoinAddress*,CAmount> pair : addressesTo){
                ...
                //add output to ohonetwork address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZOHO_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZOHO_SPEND_OKAY); // Everything okay
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zohoTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zohoTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zohoTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
            LogPrintf("%s : failed to unarchive deterministic mint %s\n", __func__, dMint.GetPubcoinHash().GetHex());
        } else {
            zohoTracker->Add(dMint, true);
        }
        listDMintsRestored.emplace_back(dMint);
    }
}

std::string CWallet::GetUniqueWalletBackupName(bool fzohoAuto) const
{
    ...
    return strprintf("wallet%s.dat%s", fzohoAuto ? "-autozohobackup" : "", DateTimeStrFormat(".%Y-%m-%d-%H-%M", GetTime()));
}

void CWallet::ZOhoBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        strNewBackupName = strprintf("wallet-autozohobackup-%d.dat", i);
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozohobackup-%d.dat", i + 1);
            ...
            strNewBackupName = strprintf("wallet-autozohobackup-%d.dat", 0);
            backupPath = backupDir / strNewBackupName;
            break;
        }
    ...
    if(!GetArg("-zohobackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zohobackuppath", ""));
...
)
{
    ...
    int nStatus = ZOHO_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZOHO_WALLET_LOCKED);
        return false;
    }
    ...
    if (!CommitTransaction(wtxNew, reserveKey)) {
        ...
        nStatus = ZOHO_COMMIT_FAILED;

        //reset all mints
        for (CZerocoinMint mint : vMintsSelected) {
            uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
            zohoTracker->SetPubcoinNotUsed(hashPubcoin);
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZOHO_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZOHO_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zohoTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zohoTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZOHO_SPEND_OKAY);  // When we reach this point spending zOHO was successful
...
{
    CMintMeta meta;
    if (!zohoTracker->GetMetaFromStakeHash(hashStake, meta))
...
{
    if (zohoTracker->HasPubcoin(bnValue))
        return true;
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zohoTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zohoTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zohoTracker->UpdateState(meta);
    } else {
...
void CWallet::setZWallet(CzOHOWallet* zwallet)
{
    zwalletMain = zwallet;
    zohoTracker = std::unique_ptr<CzOHOTracker>(new CzOHOTracker(strWalletFile));
}

CzOHOWallet* CWallet::getZWallet()
{
    return zwalletMain;
}
...
void CWallet::setZOhoAutoBackups(bool fEnabled)
{
...
{
    return GetStakeDelegationDebit(true) > 0 || GetLiteNodeStakingDebit(true) > 0;
}
