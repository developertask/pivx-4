...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 OHO at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zOHO send
enum ZerocoinSpendStatus {
    ZOHO_SPEND_OKAY = 0,                            // No error
    ZOHO_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZOHO_WALLET_LOCKED = 2,                         // Wallet was locked
    ZOHO_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZOHO_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZOHO_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZOHO_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZOHO_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZOHO_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZOHO_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZOHO_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZOHO_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZOHO_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZOHO_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZOHO_SPENT_USED_ZOHO = 14,                      // Coin has already been spend
    ZOHO_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZOHO_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
{
private:
    bool SelectCoins(const CAmount& nTargetValue, std::set<std::pair<const CWalletTx*, unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl = NULL, AvailableCoinsType coin_type = ALL_COINS, bool useIX = true, bool fIncludeLiteNodeStaking=false, bool fIncludeDelegated=true) const;
...
public:
    ...
    void ZOhoBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZOHOOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzohoAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzOHOTracker> zohoTracker;
    ...
    void setZWallet(CzOHOWallet* zwallet);
    CzOHOWallet* getZWallet();
    bool isZeromintEnabled();
    void setZOhoAutoBackups(bool fEnabled);
    ...
    void AvailableCoins(std::vector<COutput>& vCoins, bool fOnlyConfirmed = true, const CCoinControl* coinControl = NULL, bool fIncludeZeroValue = false, AvailableCoinsType nCoinType = ALL_COINS, bool fUseIX = false, int nWatchonlyConfig = 1, bool fIncludeLiteNodeStaking=false, bool fIncludeDelegated=true) const;
    ...
    /// Get 10000 OHO output and keys which can be used for the Masternode
    ...
    CAmount GetLiteNodeStakingBalance() const;  // delegated coins for which we have the staking key
    CAmount GetImmatureLiteNodeStakingBalance() const;
    CAmount GetStakingBalance(const bool fIncludeLiteNodeStaking = true) const;
    ...
    /** zOHO reset */
    boost::signals2::signal<void()> NotifyzOHOReset;
...
public:
    ...
    CAmount GetLiteNodeStakingCredit(bool fUseCache = true) const;
    CAmount GetLiteNodeStakingDebit(bool fUseCache = true) const;
