...
CZOhoStake::CZOhoStake(const libzerocoin::CoinSpend& spend)
{
...
int CZOhoStake::GetChecksumHeightFromMint()
{
...
int CZOhoStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZOhoStake::GetChecksum()
{
    return nChecksum;
}

// The zOHO block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZOhoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CZOhoStake::GetModifier(uint64_t& nStakeModifier)
{
    ...
    // zOHO staking is disabled long before block v7 (and checkpoint is not included in blocks since v7)
...
CDataStream CZOhoStake::GetUniqueness()
{
    //The unique identifier for a zOHO is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZOhoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZOhoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zOHO that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZOHOOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zOHO output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zOHO", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZOHOOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zOHO output", __func__);
...
bool CZOhoStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZOhoStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzOHOTracker* zohoTracker = pwallet->zohoTracker.get();
    CMintMeta meta;
    if (!zohoTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zohoTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}

//!OHO Stake
bool COhoStake::SetInput(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool COhoStake::GetTxFrom(CTransaction& tx)
{
    tx = txFrom;
    return true;
}

bool COhoStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount COhoStake::GetValue()
{
    return txFrom.vout[nPosition].nValue;
}

bool COhoStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
    ...
    if (whichType == TX_PUBKEYHASH) {
        ...
        // if P2CS, check that we have the litenodestaking private key
...
bool COhoStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream COhoStake::GetUniqueness()
{
    //The unique identifier for a OHO stake is the outpoint
...
CBlockIndex* COhoStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
    uint256 hashBlock = 0;
