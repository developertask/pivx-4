...
*Setup instructions for a gitian build of OHONETWORK Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the OHONETWORK
...
to the OHONETWORK GitHub release page.
...
- [Building OHONETWORK Core](#building-ohonetwork-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/ohonetwork-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/ohonetwork-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/ohonetwork-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/ohonetwork-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/ohonetwork-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/ohonetwork-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/ohonetwork-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@ohonetwork.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/ohonetwork-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/ohonetwork-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/ohonetwork-osx-*-build.assert
```
