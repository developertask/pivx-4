...
- [Development guidelines](#development-guidelines)
    - [General OHONETWORK Core](#general-ohonetwork-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
OHONETWORK Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play OHOs (tOHO)" on the test network, if you
...
OHONETWORK Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/ohonetwork-project/ohonetwork/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_ohonetwork
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_ohonetwork --log_level=test_suite
$ valgrind -v --leak-check=full src/ohonetworkd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_ohonetwork.coverage/index.html`.
```
...
- BitcoinMiner : Generates OHOs (if wallet is enabled).
...
However, in open source software such as OHONETWORK Core, where everyone uses
...
pay attention to for reviewers of OHONETWORK Core code.

General OHONETWORK Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: OHONETWORK Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against OHONETWORK Core so that they can be integrated
...
file descriptors it will cause problems with OHONETWORK's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:OHONETWORK-Project/OHONETWORK.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `ohonetwork-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `ohonetwork-cli` and the GUI debug console use this table to determine how to
